...
if ( ! class_exists( 'Cstmdmnpg_Pages_List' ) ) {
	class Cstmdmnpg_Pages_List extends WP_List_Table {
		...
		*/
		function no_items() {
			$message = isset( $_REQUEST['status'] ) && 'trash' == $_REQUEST['status'] ? __( 'No pages in trash', 'custom-admin-page' ) : __( 'No pages yet', 'custom-admin-page' ); ?>
		...
		function column_default( $item, $column_name ) {
			...
			switch ( $column_name ) {
				case 'parent':
					...
					return;
				case 'page_title':
					$page_title = empty( $item['page_title'] ) ? '<i>- ' . __( 'Empty title', 'custom-admin-page' ) . ' -</i>' : '<strong><a href="' . wp_nonce_url( '?page=custom-admin-page.php&cstmdmnpg_tab_action=edit&cstmdmnpg_page_id=' . $list['id'], 'custom-admin-page-edit' . $list['id'] ) . '">' . $item['page_title'] . '</a></strong>';
	...
	function cstmdmnpg_handle_action() {
		...
		if ( $action && check_admin_referer( $nonce_action, $nonce_query_arg ) ) {
			...
			$list_messages = array(
				...
				'add'      => __( 'Page data saved. Refresh the page to see changes.', 'custom-admin-page' ),
				'update'   => __( 'Page updated. Refresh the page to see changes.', 'custom-admin-page' ),
				...
				'empty_id' => __( 'No pages was selected', 'custom-admin-page' )
			);		
		...
		<form id="cstmdmnpg_page_form" method="post">
			<?php if ( isset( $_REQUEST['cstmdmnpg_tab_action'] ) && in_array( $_REQUEST['cstmdmnpg_tab_action'], array( 'new', 'add', 'update', 'edit' ) ) ) {
				...
				if ( $display_empty_page ) {
					$title          = __( 'Add New page', 'custom-admin-page' );
					$button_title   = __( 'Save page', 'custom-admin-page' );
					$tab_action     = 'add';
					$page_id 		= 0;
				...
				<div id="titlediv">
					<div id="titlewrap">
						<input type="text" name="cstmdmnpg_page_title" size="30" value="<?php echo $page_title; ?>" id="title" placeholder="<?php _e( 'Enter Page Title', 'custom-admin-page' ); ?>" />
					</div>
			...
			<?php } else {
				...
				echo '<h2 class="screen-reader-text">' . __( 'Filter pages list', 'custom-admin-page' ) . '</h2>';
